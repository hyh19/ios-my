...
@interface ZWSMPageControl ()
@end

@implementation ZWSMPageControl
{
@private
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (point.x < middle) {
        [self setCurrentPage:self.currentPage - 1 sevent:YES canDefer:YES];
    } else {
        [self setCurrentPage:self.currentPage + 1 sevent:YES canDefer:YES];
    }
}
...
{
    [self setCurrentPage:currentPage sevent:NO canDefer:NO];
}

- (void)setCurrentPage:(NSInteger)currentPage sevent:(BOOL)sevent canDefer:(BOOL)defer
{
