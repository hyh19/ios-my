 ...
 *  @brief  需要添加红点的索引
 */
@property (nonatomic, strong) NSMutableIndexSet *redPointsSet;
...
- (NSMutableIndexSet *)redPointsSet {
    if (!_redPointsSet) {
        _redPointsSet = [NSMutableIndexSet indexSet];
    }
    return _redPointsSet;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
             ...
             *  @brief  添加红点
             */
            if ([self.redPointsSet containsIndex:idx]) {
                CALayer *redPoint = [CALayer layer];
                redPoint.frame = CGRectMake(CGRectGetWidth(titleLayer.frame)-4, 0, 4, 4);
                redPoint.backgroundColor = [UIColor redColor].CGColor;
                redPoint.cornerRadius = CGRectGetWidth(redPoint.frame)/2;
                redPoint.masksToBounds = YES;
                [titleLayer addSublayer:redPoint];
            }
    ...
    if (self.selectedSegmentIndex != HMSegmentedControlNoSegment) {
        if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                 ...
                 *  @brief  左右两边往里缩10dp
                 */
                self.selectionIndicatorStripLayer.frame = CGRectMake(rect.origin.x+10, rect.origin.y, rect.size.width-20, rect.size.height);
...
- (void)addRedPointAtIndex:(NSUInteger)index {
    if (HMSegmentedControlTypeText == self.type &&
        index < [self.sectionTitles count] &&
        ![self.redPointsSet containsIndex:index]) {
        [self.redPointsSet addIndex:index];
        [self setNeedsDisplay];
    }
}
...
- (void)removeRedPointAtIndex:(NSUInteger)index {
    if (HMSegmentedControlTypeText == self.type &&
        index < [self.sectionTitles count] &&
        [self.redPointsSet containsIndex:index]) {
        [self.redPointsSet removeIndex:index];
        [self setNeedsDisplay];
    }
}
