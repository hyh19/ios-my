...
/** 广告数据 */
@property (nonatomic, strong) FBBannerModel *banner;
...
- (void)setBanner:(FBBannerModel *)banner {
    _banner = banner;
    __weak typeof(self) wself = self;
    
    // 解决url链接里带有中文，图片无法显示出来的问题
    NSString *urlString = [self.banner.imageURL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    [self.imageView sd_setImageWithURL:[NSURL URLWithString:urlString]
                      placeholderImage:[UIImage imageNamed:kLogoBanner]
                             completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                                 if (!error) {
                                     if (image) {
                                         CGImageRef imageRef = image.CGImage;
                                         CGSize imageSize = CGSizeMake(CGImageGetWidth(imageRef), CGImageGetHeight(imageRef));
                                         if (wself.doCompleteAction) {
                                             wself.doCompleteAction(imageSize);
                                         }
...
@interface FBBannerViewsContainer () <UIScrollViewDelegate>
{
...
@implementation FBBannerViewsContainer
