...
#define OUTPUT_FILE_SWITCH_POINT (20 * 1024 * 1024)  // 20 MB switch point
...
- (void) encodeFrame:(CMSampleBufferRef) sampleBuffer tranformImg:(CGImageRef)img
{
    @synchronized(self)
    {
        if (_needParams)
        {
            ...
            _needParams = NO;
            if ([_headerWriter encodeFrame:sampleBuffer tranformImg:img])
            {
                __weak typeof(self)wSelf = self;
                [_headerWriter finishWithCompletionHandler:^{
                    [wSelf onParamsCompletion];
                }];
            }
        }
    }
            ...
            if (st.st_size > OUTPUT_FILE_SWITCH_POINT || self.bitrateChanged)
            {
                ...
                dispatch_async(_readQueue, ^{
                    ...
                    [oldVideo finishWithCompletionHandler:^{
                        [wSelf swapFiles:oldVideo.path];
                    }];
                });
            }
        }
        [_writer encodeFrame:sampleBuffer tranformImg:img];
    }
}
